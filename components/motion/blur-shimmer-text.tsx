...
export function BlurShimmerText({
  ...
  const duration = (transition as { duration?: number })?.duration ?? 0.6;

  return (
    <div className={cn("inline-grid", className)}>
      {texts.map((text, index) => (
        <span
          // biome-ignore lint/suspicious/noArrayIndexKey: string position index required for layout measurement slots
        ...
        >
          {text.split("").map((char, i) => (
            <span
              // biome-ignore lint/suspicious/noArrayIndexKey: character position index required for split text slots
              key={`hidden-char-${i}-${char}`}
      ...
      <AnimatePresence>
        ...
        >
          {texts[currentIndex].split("").map((char, i) => {
            ...
            return (
              <motion.span
                // biome-ignore lint/suspicious/noArrayIndexKey: character position index required for staggered letter animations
                key={`active-char-${i}-${char}`}
